...
class Builtins:
    ...
    @staticmethod
    def Sqrt(value):
        """Compute the square root of a number using SQLite's ``SQRT()`` function.

        Generates a ``SQRT(<expr>)`` expression. The result is always a
        REAL (float) — even ``SQRT(4)`` returns ``2.0``, not ``2``. This is
        consistent with SQLite's decision to keep all math functions in the
        floating-point domain.

        Negative inputs produce ``NULL`` in SQLite rather than raising an
        error. If your application needs to distinguish "no result" from
        "zero", wrap the call or add a ``CHECK`` constraint on the column.

        .. note::
            ``SQRT()`` requires SQLite >= 3.35. On older libraries, use the
            portable fallback ``Builtins.Func('POW', value, 0.5)``.
        ...
        Example:
            ...
            Applied to a raw literal::
                ...
                rows = points.get_row([
                    Builtins.Sqrt(16),    # -> (SQRT(?)) with param 16 -> 4.0
    ...
    @staticmethod
    def Pow(base, exp):
        """Raise a base value to a power using SQLite's ``POW(a, b)`` function.
        ...
        ``8.0``, not ``8``, because SQLite keeps all math functions in the
        floating-point domain.

        .. note::
            ``POW()`` requires SQLite >= 3.35. On older libraries, use the
            portable fallback ``exp * Builtins.Func('LOG', base)`` after
            registering a ``LOG`` SQL function via ``conn.create_function``,
            or upgrade SQLite.
        ...
        Example:
            Compound interest — principal times ``(1 + rate)^years``::
                ...
                # SELECT [accounts].[id],
                #        ([accounts].[principal] * (POW((? + [accounts].[rate]),
                #                                       [accounts].[years])))
                # FROM [accounts]
                # Parameters: [1]
            ...
            Square or cube a value::
                ...
                # SELECT (POW([accounts].[principal], ?)),
                #        (POW([accounts].[principal], ?))
                # FROM [accounts]

            Square root via fractional exponent — equivalent to
            :meth:`Sqrt` on SQLite versions that have ``POW`` but not
            ``SQRT``::

                rows = accounts.get_row([
                    Builtins.Pow(accounts.principal, 0.5),
                ])
                # SELECT (POW([accounts].[principal], ?)) FROM [accounts]
                # Parameters: [0.5]
    ...
    @staticmethod
    def Reverse(value):
        """Reverse the characters of a text value using SQLite's ``REVERSE()``.
        ...
        .. note::
            ``REVERSE()`` requires SQLite >= 3.44 (released 2023-11-01). On
            older libraries the generated SQL will fail at execution time.
            If you must support older SQLite, reverse the string in Python
            after fetching, or register a custom SQL function via
        ...
        Args:
            value: The expression whose characters are reversed. Supported
                ...
                - :class:`ColumnsOperation` — its SQL fragment and parameters
                  are reused.
                - :class:`Column` — the fully qualified column name is used.
                - A raw Python string — bound as a ``?`` placeholder.
        ...
        Example:
            ...
            Suffix search turned into prefix search — reverse both sides
            so that ``endswith`` becomes ``startswith``::
                ...
                # SELECT [users].[username] FROM [users]
                # WHERE ((REVERSE([users].[username])) LIKE ? || '%')
                # Parameters: ['e']
            ...
            Applied to a raw literal::
                ...
                rows = users.get_row([
                    Builtins.Reverse('hello'),    # -> (REVERSE(?)) with param 'hello'
                ])
                # -> [('olleh',)]
        """
        sql, p, _, c = Builtins._normalize(value)
    ...
    def UnixEpoch(value):
        ...
        """
        sql, p, _, c = Builtins._normalize(value)
        return Builtins._make(f"(unixepoch({sql}))", p, int, c)
    ...
    @staticmethod
    def Timediff(a, b):
        """Compute the difference between two date/time values in human-readable form.

        Generates a ``timediff(<a>, <b>)`` expression. SQLite returns a
        TEXT string describing the elapsed time between ``a`` and ``b`` in
        the format:
        ...
        Sign convention: the result is ``a - b``. A positive sign means
        ``a`` occurs after ``b``; a negative sign means ``a`` occurs before
        ``b``. When ``a`` and ``b`` are within the same year, month, and
        day, the date portion is ``0000-00-00``.

        This is the SQL analogue of Python's ``datetime.timedelta``
        formatted as a string. It is convenient for display because the
        output is directly readable, but not for arithmetic — for numeric
        differences in days or seconds, use :meth:`DateDiffDays` or
        :meth:`DateDiffSeconds` instead.

        .. note::
            ``timediff()`` requires SQLite >= 3.43 (released 2023-08-24).
            On older libraries, use
            ``Builtins.JulianDay(a) - Builtins.JulianDay(b)`` and format
            the resulting day count in Python.
        ...
        Example:
            Human-readable account age for every user::
                ...
                # SELECT [users].[username],
                #        (timediff((datetime(?)), [users].[created_at]))
                # FROM [users]
                # Parameters: ['now']
            ...
            Compare two timestamp columns::
                ...
                # -> [(1, '+0000-00-00 01:23:45.000'), ...]

            Filter by elapsed time — compare against a literal duration
            using the string ordering::

                # Note: string comparison on timediff output is
                # lexicographic, which is NOT the same as chronological.
                # For a numeric filter, use DateDiffDays / DateDiffSeconds:
                recently = (
                    Builtins.DateDiffDays(Builtins.Now(), users.created_at) < 7
                )
                rows = users.get_row([users.username], where=recently)

            Combine with Format for a prettier display::
                ...
                # -> [('Alice', 'Member for +0000-00-02 05:29:18.000'), ...]

            Sign convention — the "a - b" order matters::
                ...
                # -> [(1, '+0000-00-00 00:30:00.000'), ...]  (updated after created)
                ...
                # -> [(1, '-0000-00-00 00:30:00.000'), ...]  (created before updated)

            Applied to raw literals::

                rows = users.get_row([
                    Builtins.Timediff('2024-03-15', '2024-03-01'),
                    # -> '+0000-00-14 00:00:00.000'
    ...
    def UnixNow(_=None):
        ...
        """
        return Builtins._make("(unixepoch('now'))", [], int, Builtins._NullCol)
