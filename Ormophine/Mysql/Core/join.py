...
class JoinQuery:
    ...
    Identifier quoting
    ------------------
    MySQL/MariaDB use backticks (`` `table` ``) for identifier quoting,
    unlike PostgreSQL which uses double quotes. All generated SQL uses
    ...
    Note:
        ...
        - MySQL/MariaDB support ``RIGHT JOIN`` natively, but ``LEFT JOIN``
          is usually clearer and produces the same result when the tables
          are swapped.
    """
    ...
    def right_join(self, table, condition, alias=None) -> 'JoinQuery':
        ...
        Note:
            Both MySQL and MariaDB support ``RIGHT JOIN`` natively. In
            practice, ``LEFT JOIN`` is more common; swapping the two tables
            and using :meth:`left_join` produces the same result and is
            often clearer.
        """
    ...
    def _make_unique_alias(self, table) -> str:
        ...
        Note:
            The surrounding backticks of the table name are stripped before
            the counter suffix is appended; the alias is later re-quoted
            with backticks when it is emitted in the SQL.
        """
    ...
    def _resolve_column_ref(self, col) -> str:
        ...
        Returns:
            str: Either the plain qualified name (e.g. ``'`users`.`id`'``)
            or the aliased form (e.g. ``'`users_0`.`id`'``) when the parent
            table is aliased.
    ...
    def get_row(
        ...
        inside ``which_columns``, ``where`` and ``order_by`` are rewritten
        to use table aliases when a table participates in the join more than
        once, preventing ambiguous-column errors.

        Args:
            which_columns (list[Column | ColumnsOperation]): The columns or
                computed expressions to include in the ``SELECT`` list. Each
                element can be a :class:`Column` (returned as-is with an
                alias of the form ``<table>_<column>``) or a
                :class:`ColumnsOperation` (e.g. ``orders.total * -1``,
                ``users.username.upper()``).
            where (ColumnsOperation, optional): A filter condition. Both
                sides may reference columns from any table participating in
                the join; aliases are applied automatically. Defaults to
                ``None`` (no filter).
            order_by (Column | ColumnsOperation, optional): The expression to
                sort by. May be a plain :class:`Column` (e.g. ``users.id``)
                or a computed :class:`ColumnsOperation` (e.g.
                ``orders.total * -1``, ``users.username.upper()``).
                Descending order is expressed by multiplying the column by
                ``-1``, since the ORM has no ``DESC`` keyword. Defaults to
                ``None`` (no ordering).
            ...
            offset (int, optional): Number of rows to skip before returning
                ...
                empty list. MySQL requires ``LIMIT`` before ``OFFSET``; when
                ``offset`` is given without ``limit``, the method emits
        ...
        Returns:
            ...
            Always returns tuples (never a flat list), even when only one
            column is requested, because the join may span several tables.
        ...
        Example:
            ...
            Offset without limit (uses MySQL's ``LIMIT 18446744073709551615``
            idiom internally)::

                >>> rows = (users.inner_join(orders, orders.user_id == users.id)
                ...             .get_row([orders.amount],
                ...                      order_by=orders.amount * -1,
                ...                      offset=7))
        """
