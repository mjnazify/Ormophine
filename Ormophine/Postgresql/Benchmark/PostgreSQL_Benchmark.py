"""Benchmark utilities for comparing PostgreSQL ORM performance (Full CRUD)."""
...
from peewee import PostgresqlDatabase, Model, IntegerField, FloatField, TextField, AutoField, fn
...
def sqlalchemy_single_operations(rng):
    ...
    con = psycopg.connect(**POSTGRES_CONFIG)
    cursor = con.cursor()
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS "TestTable_alchemy" (
            ...
            "col_3" DOUBLE PRECISION DEFAULT 5.0
        );
    ''')
    con.commit()
    cursor.close()
    ...
    engine = create_engine(
        f"postgresql+psycopg://{quote_plus(POSTGRES_CONFIG['user'])}:{quote_plus(POSTGRES_CONFIG['password'])}@{POSTGRES_CONFIG['host']}:{POSTGRES_CONFIG['port']}/{POSTGRES_CONFIG['dbname']}",
        echo=False
    )
...
def peewee_single_operations(rng):
    ...
    db = PostgresqlDatabase(POSTGRES_CONFIG['dbname'], host=POSTGRES_CONFIG['host'], user=POSTGRES_CONFIG['user'], password=POSTGRES_CONFIG['password'], port=POSTGRES_CONFIG['port'])
...
def pony_single_operations(rng):
    ...
    db.bind(provider='postgres', host=POSTGRES_CONFIG['host'], user=POSTGRES_CONFIG['user'], password=POSTGRES_CONFIG['password'], dbname=POSTGRES_CONFIG['dbname'], port=POSTGRES_CONFIG['port'])
...
def Ormophine_single_operations(rng):
    ...
    db = Driver(host=POSTGRES_CONFIG['host'], username=POSTGRES_CONFIG['user'], password=POSTGRES_CONFIG['password'], db_name=POSTGRES_CONFIG['dbname'], port=POSTGRES_CONFIG['port'])
...
def run_single_operation_benchmark(repeats: int, warmup_repeats: int = 3, chunk_size: int = 10):
    ...
    print_qps_summary('PostgreSQL Single Operation Benchmark', labels, operations_data)
    plot_benchmark_results('PostgreSQL Single Operation Benchmark', labels, operations_data, queries_per_repeat=chunk_size)
    ...
    print("="*60)
    print("This benchmark evaluates the performance of 4 Python ORMs (Ormophine, Pony ORM, SQLAlchemy, Peewee) on a PostgreSQL database.")
    ...
    print(f"- PostgreSQL Server: {POSTGRES_CONFIG['host']}:{POSTGRES_CONFIG['port']} | Database: {POSTGRES_CONFIG['dbname']}")
...
def run_batch_operation_benchmark(repeats: int, warmup_repeats: int = 3, chunk_size: int = 10000):
    ...
    print_qps_summary('PostgreSQL Batch Operation Benchmark', labels, operations_data)
    plot_benchmark_results('PostgreSQL Batch Operation Benchmark', labels, operations_data, queries_per_repeat=chunk_size)
