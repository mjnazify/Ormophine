...
def driver():
    drv = Sqlite.Driver('test_orm.db', setup_time=0.1)
    ...
    try:
        drv.custom_execute('DROP TABLE test_table')
    except:
        pass
...
def test_07(col_str, lit_str):
    res = col_str + lit_str
    assert res._output[0] == "([test_table].[name] || ?)"
...
def test_47(col_str, op_int):
    ...
    assert res._output[0] == "(([test_table].[name] || [test_table].[name]) || ([test_table].[age] + ?))"
    assert res._output[1] == [1]

def test_48(col_str, op_float):
    res = (col_str + col_str) + op_float
    assert res._output[0] == "(([test_table].[name] || [test_table].[name]) || ([test_table].[score] + ?))"
    assert res._output[1] == [1.5]
...
def test_57(col_int, lit_str):
    res = col_int + lit_str
    assert res._output[0] == "([test_table].[age] || ?)"
    assert res._output[1] == ['text']
...
def test_97(col_int, op_str):
    ...
    assert res._output[0] == "(([test_table].[age] + [test_table].[age]) || ([test_table].[name] || ?))"
    assert res._output[1] == ['op_str_param']

def test_98(col_int, op_float):
    res = (col_int + col_int) + op_float
    assert res._output[0] == "(([test_table].[age] + [test_table].[age]) + ([test_table].[score] + ?))"
    assert res._output[1] == [1.5]
...
def test_107(col_float, lit_str):
    ...
    assert res._output[0] == "([test_table].[score] || ?)"
    assert res._output[1] == ['text']
...
def test_147(col_float, op_int):
    ...
    assert res._output[0] == "(([test_table].[score] + [test_table].[score]) + ([test_table].[age] + ?))"
    assert res._output[1] == [1]

def test_148(col_float, op_str):
    res = (col_float + col_float) + op_str
    assert res._output[0] == "(([test_table].[score] + [test_table].[score]) || ([test_table].[name] || ?))"
    assert res._output[1] == ['op_str_param']
...
def test_316(col_str, op_str, col_int, op_int):
    ...
    assert res._output[0] == "(([test_table].[name] || ([test_table].[name] || ?)) || ([test_table].[age] + ([test_table].[age] + ?)))"
    assert res._output[1] == ['op_str_param', 1]
...
def test_351(col_str, col_int, col_float, lit_str):
    ...
    assert res._output[0] == "((([test_table].[name] || [test_table].[age]) || [test_table].[score]) || ?)"
    assert res._output[1] == ['text']

def test_352(col_int, col_float, col_str, lit_int):
    res = ((col_int - col_float) - col_str) - lit_int
    assert res._output[0] == "((([test_table].[age] - [test_table].[score]) - [test_table].[name]) - ?)"
    assert res._output[1] == [10]
...
def test_376(lit_int, col_int, op_str, col_float):
    ...
    assert res._output[0] == "(((? - [test_table].[age]) - ([test_table].[name] || ?)) - [test_table].[score])"
    assert res._output[1] == [10, 'op_str_param']
...
def test_400(col_int, col_float, lit_int, op_str):
    ...
    assert res._output[0] == "((([test_table].[age] - [test_table].[score]) - ?) - (([test_table].[name] || ?) - [test_table].[score]))"
    assert res._output[1] == [10, 'op_str_param']
...
def test_404(lit_int, col_str, col_float, col_int, lit_str):
    ...
    assert res._output[0] == "(? - (([test_table].[name] - [test_table].[score]) - ([test_table].[age] - ?)))"
    assert res._output[1] == [10, 'text']

def test_405(col_str, col_int, col_float, lit_str):
    res = ((col_str + (col_int + col_float)) + lit_str) + col_int
    assert res._output[0] == "((([test_table].[name] || ([test_table].[age] + [test_table].[score])) || ?) || [test_table].[age])"
    assert res._output[1] == ['text']
...
def test_421(col_str, op_str, col_int, op_float, lit_str):
    ...
    assert res._output[0] == "(([test_table].[name] || ([test_table].[name] || ?)) || ([test_table].[age] || (([test_table].[score] + ?) || ?)))"
...
def test_422(col_int, op_int, col_float, op_str, lit_int):
    ...
    assert res._output[0] == "(([test_table].[age] - ([test_table].[age] + ?)) - ([test_table].[score] - (([test_table].[name] || ?) - ?)))"
...
def test_424(lit_str, col_str, op_int, col_int, op_float):
    ...
    assert res._output[0] == "(? - (([test_table].[name] - ([test_table].[age] + ?)) - ([test_table].[age] - ([test_table].[score] + ?))))"
    assert res._output[1] == ['text', 1, 1.5]
...
def test_426(col_int, op_float, col_str, lit_int, op_str):
    ...
    assert res._output[0] == "((([test_table].[age] - (([test_table].[score] + ?) - [test_table].[name])) - ?) - ([test_table].[name] || ?))"
...
def test_433(op_str, lit_int, col_float, col_int):
    ...
    assert res._output[0] == "(([test_table].[name] || ?) || ((? + [test_table].[score]) || (([test_table].[name] || ?) || [test_table].[age])))"
    assert res._output[1] == ['op_str_param', 10, 'op_str_param']

def test_434(op_int, lit_str, op_float, col_int, op_str):
    res = op_int - ((lit_str - op_float) - (col_int - op_str))
    assert res._output[0] == "(([test_table].[age] + ?) - ((? - ([test_table].[score] + ?)) - ([test_table].[age] - ([test_table].[name] || ?))))"
...
def test_436(lit_int, op_float, col_str, lit_str, col_int):
    ...
    assert res._output[0] == "(((? - (([test_table].[score] + ?) - [test_table].[name])) - ?) - [test_table].[age])"
...
def test_443(op_str, col_int, lit_float):
    ...
    assert res._output[0] == "(([test_table].[name] || ?) || (([test_table].[age] + ?) || (([test_table].[name] || ?) || [test_table].[age])))"
...
def test_444(op_int, col_str, lit_float, col_int, op_str):
    ...
    assert res._output[0] == "(([test_table].[age] + ?) - (([test_table].[name] - ?) - ([test_table].[age] - ([test_table].[name] || ?))))"
...
def test_446(col_int, lit_float, col_str, op_int, col_float):
    ...
    assert res._output[0] == "((([test_table].[age] - (? - [test_table].[name])) - ([test_table].[age] + ?)) - [test_table].[score])"
    assert res._output[1] == [10.5, 1]
...
def test_471(op_str, col_int, op_int, col_str, op_float):
    ...
    assert res._output[0] == "(((([test_table].[name] || ?) || [test_table].[age]) || (([test_table].[age] + ?) || [test_table].[name])) || (([test_table].[score] + ?) + [test_table].[age]))"
...
def test_472(op_int, col_str, op_float, col_int, op_str, col_float):
    ...
    assert res._output[0] == "(((([test_table].[age] + ?) - [test_table].[name]) - (([test_table].[score] + ?) - [test_table].[age])) - (([test_table].[name] || ?) - [test_table].[score]))"
...
def test_473(col_str, op_int, lit_str, op_float, col_int, op_str):
    ...
    assert res._output[0] == "((([test_table].[name] || ([test_table].[age] + ?)) || (? || ([test_table].[score] + ?))) || ([test_table].[age] || ([test_table].[name] || ?)))"
    assert res._output[1] == [1, 'text', 1.5, 'op_str_param']

def test_474(col_int, op_str, lit_int, op_float, col_float, op_int):
    res = (col_int - op_str) - (lit_int - op_float) - (col_float - op_int)
    assert res._output[0] == "((([test_table].[age] - ([test_table].[name] || ?)) - (? - ([test_table].[score] + ?))) - ([test_table].[score] - ([test_table].[age] + ?)))"
...
def test_475(lit_str, col_int, op_str, col_float, lit_int, col_str):
    ...
    assert res._output[0] == "(((? || [test_table].[age]) || (([test_table].[name] || ?) || [test_table].[score])) || (? || [test_table].[name]))"
...
def test_477(col_str, col_int, col_float, op_str, op_int, op_float):
    ...
    assert res._output[0] == "([test_table].[name] || ([test_table].[age] || ([test_table].[score] || (([test_table].[name] || ?) || (([test_table].[age] + ?) + ([test_table].[score] + ?))))))"
    assert res._output[1] == ['op_str_param', 1, 1.5]

def test_478(col_int, col_float, col_str, op_int, op_float, op_str):
    res = col_int - (col_float - (col_str - (op_int - (op_float - op_str))))
    assert res._output[0] == "([test_table].[age] - ([test_table].[score] - ([test_table].[name] - (([test_table].[age] + ?) - (([test_table].[score] + ?) - ([test_table].[name] || ?))))))"
    assert res._output[1] == [1, 1.5, 'op_str_param']
...
def test_482(lit_int, col_float, op_str, col_int, op_float, lit_str):
    ...
    assert res._output[0] == "(? - ([test_table].[score] - (([test_table].[name] || ?) - ([test_table].[age] - (([test_table].[score] + ?) - ?)))))"
...
def test_485(op_str, col_int, op_int, col_str, op_float, col_float):
    ...
    assert res._output[0] == "(((([test_table].[name] || ?) || [test_table].[age]) || (([test_table].[age] + ?) || [test_table].[name])) || ((([test_table].[score] + ?) + [test_table].[age]) || (([test_table].[name] || ?) || [test_table].[score])))"
    assert res._output[1] == ['op_str_param', 1, 1.5, 'op_str_param']
...
def test_486(op_int, col_str, op_float, col_int, op_str, col_float):
    ...
    assert res._output[0] == "(((([test_table].[age] + ?) - [test_table].[name]) - (([test_table].[score] + ?) - [test_table].[age])) - ((([test_table].[name] || ?) - [test_table].[score]) - (([test_table].[age] + ?) - [test_table].[name])))"
    assert res._output[1] == [1, 1.5, 'op_str_param', 1]
...
def test_487(col_str, op_int, lit_str, op_float, col_int, op_str, lit_int):
    ...
    assert res._output[0] == "((([test_table].[name] || ([test_table].[age] + ?)) || (? || ([test_table].[score] + ?))) || (([test_table].[age] || ([test_table].[name] || ?)) || (? + ([test_table].[age] + ?))))"
...
def test_488(col_int, op_str, lit_int, op_float, col_float, op_int, lit_float):
    ...
    assert res._output[0] == "((([test_table].[age] - ([test_table].[name] || ?)) - (? - ([test_table].[score] + ?))) - (([test_table].[score] - ([test_table].[age] + ?)) - (? - ([test_table].[name] || ?))))"
...
def test_489(lit_str, col_int, op_str, col_float, lit_int, col_str, op_int):
    ...
    assert res._output[0] == "(((? || [test_table].[age]) || (([test_table].[name] || ?) || [test_table].[score])) || ((? || [test_table].[name]) || (([test_table].[age] + ?) + [test_table].[score])))"
...
def test_491(col_str, col_int, col_float, op_str, op_int, op_float, lit_int):
    ...
    assert res._output[0] == "(((([test_table].[name] || [test_table].[age]) || [test_table].[score]) || ([test_table].[name] || ?)) || (((([test_table].[age] + ?) + ([test_table].[score] + ?)) || [test_table].[name]) || ?))"
...
def test_492(col_int, col_float, col_str, op_int, op_float, op_str, lit_str):
    ...
    assert res._output[0] == "(((([test_table].[age] - [test_table].[score]) - [test_table].[name]) - ([test_table].[age] + ?)) - (((([test_table].[score] + ?) - ([test_table].[name] || ?)) - [test_table].[age]) - ?))"
...
def test_494(lit_int, col_float, col_str, op_int, op_float, op_str, col_int, lit_str):
    ...
    assert res._output[0] == "(? - ((([test_table].[score] - [test_table].[name]) - ([test_table].[age] + ?)) - (((([test_table].[score] + ?) - ([test_table].[name] || ?)) - [test_table].[age]) - ?)))"
...
def test_495(col_str, col_int, col_float, op_str, lit_int, col_float_2=col_float):
    ...
    assert res._output[0] == "((((([test_table].[name] || [test_table].[age]) || [test_table].[score]) || ([test_table].[name] || ?)) || ?) || [test_table].[score])"
    assert res._output[1] == ['op_str_param', 10]
...
def test_496(col_int, col_float, col_str, op_int, lit_str, col_float_2=col_float):
    ...
    assert res._output[0] == "((((([test_table].[age] - [test_table].[score]) - [test_table].[name]) - ([test_table].[age] + ?)) - ?) - [test_table].[score])"
    assert res._output[1] == [1, 'text']
...
def test_556(col_str):
    ...
    assert res._output[0] == "([test_table].[name] like ? || '%')"
...
def test_559(col_str):
    ...
    assert res._output[0] == "((upper([test_table].[name])) like ? || '%')"
    assert res._output[1] == ['A']

def test_560(col_str):
    res = col_str.lower().endswith('z')
    assert res._output[0] == "((lower([test_table].[name])) like '%' || ?)"
    assert res._output[1] == ['z']
...
def test_570(col_str, op_str):
    ...
    assert res._output[0] == "(upper(([test_table].[name] || ([test_table].[name] || ?))))"
    assert res._output[1] == ['op_str_param']

def test_571(col_str, op_str):
    res = (col_str + op_str).startswith('A')
    assert res._output[0] == "(([test_table].[name] || ([test_table].[name] || ?)) like ? || '%')"
    assert res._output[1] == ['op_str_param', 'A']

def test_572(col_str, op_str):
    res = (col_str + op_str).upper().startswith('A')
    assert res._output[0] == "((upper(([test_table].[name] || ([test_table].[name] || ?)))) like ? || '%')"
    assert res._output[1] == ['op_str_param', 'A']

def test_573(op_str, col_str):
    res = (op_str + col_str).lower().endswith('z')
    assert res._output[0] == "((lower((([test_table].[name] || ?) || [test_table].[name]))) like '%' || ?)"
    assert res._output[1] == ['op_str_param', 'z']
...
def test_577(col_str, col_int):
    ...
    assert res._output[0] == "(([test_table].[name] || [test_table].[age]) like ? || '%')"
    assert res._output[1] == ['A']
...
def test_582(col_str, op_str):
    ...
    assert res._output[0] == "((upper([test_table].[name])) || (lower(([test_table].[name] || ?))))"
    assert res._output[1] == ['op_str_param']

def test_583(col_str, op_str):
    res = col_str.startswith('A') + op_str.endswith('Z')
    assert res._output[0] == "(([test_table].[name] like ? || '%') || (([test_table].[name] || ?) like '%' || ?))"
...
def test_590(col_str):
    ...
    assert res._output[0] == "(([test_table].[name] like ? || '%') like ? || '%')"
...
def test_592(col_str, lit_str):
    ...
    assert res._output[0] == "((lower((upper((? || [test_table].[name]))))) like ? || '%')"
    assert res._output[1] == ['text', 'a']
...
def test_598(op_str, op_int):
    ...
    assert res._output[0] == "((upper((([test_table].[name] || ?) || ([test_table].[age] + ?)))) like '%' || ?)"
    assert res._output[1] == ['op_str_param', 1, 'Z']

def test_599(col_str, op_float):
    res = (col_str + op_float).lower().like('test')
    assert res._output[0] == "((lower(([test_table].[name] || ([test_table].[score] + ?)))) like ?)"
...
def test_604(col_str, op_str):
    ...
    assert res._output[0] == "((upper([test_table].[name])) || (upper(([test_table].[name] || ?))))"
    assert res._output[1] == ['op_str_param']
...
def test_667(col_int, lit_int):
    ...
    assert res._output[0] == "(([test_table].[age] > ?) AND ([test_table].[age] < ?))"
...
def test_675(col_str, lit_str):
    res = (col_str == lit_str) | (col_str == 'other')
    assert res._output[0] == "(([test_table].[name] = ?) OR ([test_table].[name] = ?))"
    assert res._output[1] == ['text', 'other']

def test_676(col_str, lit_str):
    res = (col_str.contains('A')) | (col_str.startswith('B'))
    assert res._output[0] == "(([test_table].[name] like '%' || ? || '%') OR ([test_table].[name] like ? || '%'))"
    assert res._output[1] == ['A', 'B']

def test_677(col_int, col_float):
    res = (col_int > 5) | (col_float < 10.0)
    assert res._output[0] == "(([test_table].[age] > ?) OR ([test_table].[score] < ?))"
    assert res._output[1] == [5, 10.0]

def test_678(col_int, col_str, lit_str, lit_int):
    res = (col_int == lit_int) | (col_str == lit_str)
    assert res._output[0] == "(([test_table].[age] = ?) OR ([test_table].[name] = ?))"
    assert res._output[1] == [10, 'text']

def test_679(col_int, col_str, lit_int, lit_str):
    res = ((col_int > lit_int) & (col_str == lit_str)) | (col_int == 0)
    assert res._output[0] == "((([test_table].[age] > ?) AND ([test_table].[name] = ?)) OR ([test_table].[age] = ?))"
    assert res._output[1] == [10, 'text', 0]

def test_680(col_int, col_float):
    res = (col_int > 5) & ((col_float < 10.0) | (col_float > 20.0))
    assert res._output[0] == "(([test_table].[age] > ?) AND (([test_table].[score] < ?) OR ([test_table].[score] > ?)))"
    assert res._output[1] == [5, 10.0, 20.0]

def test_681(col_str, col_int):
    res = (col_str == 'A') | ((col_int > 5) & (col_int < 20))
    assert res._output[0] == "(([test_table].[name] = ?) OR (([test_table].[age] > ?) AND ([test_table].[age] < ?)))"
...
def test_701(col_str):
    res = col_str[:3] == 'abc'
    assert res._output[0] == "((substr([test_table].[name] , 0 , ?)) = ?)"
    assert res._output[1] == [4, 'abc']  

def test_702(col_str):
    res = col_str[1:4] != 'def'
    assert res._output[0] == "((substr([test_table].[name] , ? , ?)) != ?)"
    assert res._output[1] == [2, 3, 'def']  

def test_703(col_str):
    res = col_str.replace('a', 'b') == 'xyz'
    assert res._output[0] == "((replace([test_table].[name] , ? , ?)) = ?)"
    assert res._output[1] == ['a', 'b', 'xyz']

def test_704(col_str):
    res = col_str.strip() == 'text'
    assert res._output[0] == '((trim([test_table].[name]," ")) = ?)'
    assert res._output[1] == ['text']

def test_705(col_int, col_str):
    res = ((col_int > 5) & (col_int < 20)) | (col_str == 'admin')
    assert res._output[0] == "((([test_table].[age] > ?) AND ([test_table].[age] < ?)) OR ([test_table].[name] = ?))"
...
def test_707(op_str, op_int):
    ...
    assert res._output[0] == "((([test_table].[name] || ?) = ?) AND (([test_table].[age] + ?) > ?))"
    assert res._output[1] == ['op_str_param', 'test', 1, 5]

def test_708(op_float, op_int):
    res = (op_float < 10.0) | (op_int == 1)
    assert res._output[0] == "((([test_table].[score] + ?) < ?) OR (([test_table].[age] + ?) = ?))"
...
def test_711(col_str, lit_str):
    ...
    assert res._output[0] == "(([test_table].[name] like ?) AND ([test_table].[name] != ?))"
    assert res._output[1] == ['%test%', '']

def test_712(col_str, lit_str):
    res = (col_str.contains('A')) | (col_str == lit_str)
    assert res._output[0] == "(([test_table].[name] like '%' || ? || '%') OR ([test_table].[name] = ?))"
    assert res._output[1] == ['A', 'text']

def test_713(col_int, col_float):
    res = (col_int > 5) & (col_float < 10.0) | (col_int == 0)
    assert res._output[0] == "((([test_table].[age] > ?) AND ([test_table].[score] < ?)) OR ([test_table].[age] = ?))"
