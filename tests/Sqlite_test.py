...
def users_if(if_driver):
    ...
    schema.add_column('active', Sqlite.DataTypes.BOOLEAN())
    tbl = if_driver.create_table(schema)
    tbl.bulk_insert(
...
def test_if_06_nested_conditionals(users_if):
    ...
    label = (
        ...
        .If(users_if.active == 0).Else('active')
    )
...
def test_if_08_else_on_column_raises(users_if):
    """`.Else()` directly on a Column raises a clear RuntimeError."""
    with pytest.raises(RuntimeError, match="must be chained after"):
        users_if.name.Else('x')


def test_if_09_else_on_columns_operation_raises(users_if):
    """`.Else()` on a plain ColumnsOperation also raises."""
    op = users_if.name.upper()   # plain ColumnsOperation
    with pytest.raises(RuntimeError, match="must be chained after"):
        op.Else('x')
...
def test_if_22_conditional_in_order_by(users_if):
    ...
    label = users_if.name.If(users_if.name == None).Else(Sqlite.LiteralValue('zzz'))
...
def test_if_27_multiple_conditionals_in_select(users_if):
    ...
    a = users_if.name.If(users_if.active == 1).Else('X')
    b = users_if.age.If(users_if.age == None).Else(Sqlite.LiteralValue(-1))
    res = users_if.get_row([a, b], order_by=users_if.id)
    assert res == [
        ('Ali', 30),
        ('Reza', 17),
...
def test_if_29_conditional_in_join(if_driver, users_if):
    ...
    orders = if_driver.create_table(schema)
    orders.bulk_insert(
...
#     schema = Sqlite.TableStructure(name, strict=True)
#     schema.add_column("name", Sqlite.DataTypes.TEXT(max_length=100))
#     schema.add_column("age", Sqlite.DataTypes.INTEGER())
#     return driver.create_table(schema)
# @pytest.fixture
# def fresh_admins(driver):
#     import uuid
#     name = f"admins_{uuid.uuid4().hex[:8]}"
#     schema = Sqlite.TableStructure(name, strict=True)
#     schema.add_column("username", Sqlite.DataTypes.TEXT())
#     schema.add_column("active", Sqlite.DataTypes.BOOLEAN())
#     return driver.create_table(schema)
# def test_in_functional(fresh_table):
#     fresh_table.bulk_insert(
#         [fresh_table.name, fresh_table.age],
#         [("Alice", 30), ("Bob", 25), ("Carol", 28), ("Dave", 22)],
#     )
#     res = fresh_table.get_row(
#         [fresh_table.name], fresh_table.name.In(["Alice", "Carol"])
#     )
#     assert set(res) == {"Alice", "Carol"}
# def test_not_in_functional(fresh_table):
#     fresh_table.bulk_insert(
#         [fresh_table.name, fresh_table.age],
#         [("X1", 1), ("X2", 2), ("X3", 3), ("X4", 4)],
#     )
#     res = fresh_table.get_row(
#         [fresh_table.name],
#         fresh_table.name.not_In(["X1", "X3"]),
#         order_by=fresh_table.name,
#     )
#     assert res == ["X2", "X4"]
# def test_in_subquery_functional(driver, fresh_table, fresh_admins):
#     fresh_table.bulk_insert(
#         [fresh_table.name, fresh_table.age],
#         [("admin1", 30), ("user1", 25), ("admin2", 28), ("user2", 22)],
#     )
#     fresh_admins.bulk_insert(
#         [fresh_admins.username, fresh_admins.active],
#         [("admin1", 1), ("admin2", 1), ("admin3", 0)],
#     )
#     res = fresh_table.get_row(
#         [fresh_table.name],
#         fresh_table.name.In(
#             column=fresh_admins.username,
#             where=fresh_admins.active == 1,
#         ),
#         order_by=fresh_table.name,
#     )
#     assert res == ["admin1", "admin2"]
# def test_not_in_subquery_functional(driver, fresh_table, fresh_admins):
#     fresh_table.bulk_insert(
#         [fresh_table.name, fresh_table.age],
#         [("a1", 1), ("b1", 2), ("c1", 3), ("d1", 4)],
#     )
...
#     )
#     assert res == ["b1", "d1"]
# def test_eq_none_functional(driver, fresh_table):
#     fresh_table.bulk_insert(
#         [fresh_table.name, fresh_table.age],
#         [("has_name", 1), (None, 2), (None, 3)],
#     )
#     res = fresh_table.get_row(
#         [fresh_table.age], fresh_table.name == None, order_by=fresh_table.age
#     )
#     assert res == [2, 3]
# def test_ne_none_functional(driver, fresh_table):
#     fresh_table.bulk_insert(
#         [fresh_table.name, fresh_table.age],
...
#     )
#     assert res == [1, 3]
# def test_eq_none_on_int_functional(driver, fresh_table):
#     fresh_table.bulk_insert(
#         [fresh_table.name, fresh_table.age],
#         [("a", 10), ("b", None), ("c", 30)],
#     )
#     assert fresh_table.get_row([fresh_table.name], fresh_table.age == None) == ["b"]
#     assert fresh_table.get_row(
#         [fresh_table.name],
#         fresh_table.age != None,
#         order_by=fresh_table.name,
#     ) == ["a", "c"]
# def test_eq_none_on_columns_operation_functional(driver, fresh_table):
#     fresh_table.bulk_insert(
#         [fresh_table.name, fresh_table.age],
#         [("a", 1), (None, 2)],
#     )
...
#     op = fresh_table.name + "!"
#     res = fresh_table.get_row([fresh_table.age], op == None, order_by=fresh_table.age)
#     assert res == [2]
# def test_ne_none_on_columns_operation_functional(driver, fresh_table):
#     fresh_table.bulk_insert(
#         [fresh_table.name, fresh_table.age],
#         [("a", 1), (None, 2), ("c", 3)],
#     )
...
#     res = fresh_table.get_row([fresh_table.age], op != None, order_by=fresh_table.age)
#     assert res == [1, 3]
# def test_in_with_null_is_null_functional(driver, fresh_table):
    
#     fresh_table.bulk_insert(
#         [fresh_table.name, fresh_table.age],
#         [("A", None), ("B", 20), ("A", 30)],
#     )
#     cond = (fresh_table.name.In(["A"])) & (fresh_table.age == None)
#     res = fresh_table.get_row([fresh_table.age], cond)
#     assert res == [None]
# @pytest.fixture
