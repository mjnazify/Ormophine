...
def test_256_bulk_update_one_placeholder(users, driver):
    ...
    users.bulk_update({users.name: users.PLACE_HOLDER}, users.id == 1, [('New',)])
...
def test_257_bulk_update_two_placeholders(users, driver):
    ...
    users.bulk_update({users.name: users.PLACE_HOLDER}, users.id == users.PLACE_HOLDER, [('Updated1', 1), ('Updated2', 2)])
...
def test_260_bulk_update_empty_list(users, driver):
    ...
    users.insert({users.id: 1, users.name: 'A'})
    users.bulk_update({users.name: users.PLACE_HOLDER}, users.id == 1, [])
    res = users.get_row([users.name], users.id == 1)
    assert res[0] == 'A' 
@pytest.fixture
def db_path(tmp_path):
...
def test_387_batch_on_conflict_placeholder(users, driver):
    ...
    users.bulk_update({users.name: users.PLACE_HOLDER}, users.id == 1, [('FromBulk',)])
    res = users.get_row([users.name], users.id == 1)
